...
class RawEmailMessage {
	...
	/**
	 * Get all the headers of the message as a whole (note: header names are lowercase)
	 *
	...
	protected function ExtractHeadersAndRawBody($aLines, int $iDepth) {
		...
		foreach($aLines as $sLine) {
			...
			if(self::IsLineStartingWithPrintableChar($sLine)) {
				// Start of new header
				...
				// The current header continues on this line.
	...
	protected static function ExtractAddressPieces($sAddress) {
		...
		// Combodo's original implementation indeed assumes that there should be no spaces.
		...
		// This fix still processes invalid From: headers where there are spaces before, after, or
		// within the email address (e.g. "<name @domain.com>") - the name portion outside the
		// angle brackets is still reliably extractable even when the email itself is malformed.
		elseif(preg_match('/^(.*)<(.+)>$/', $sAddress, $aMatches)) {
			$sName = trim($aMatches[1], ' "');
			$sEmail = trim($aMatches[2]);
		}
		else {
			if(preg_match('/^([^ ]+) ?\((.*)\)$/', $sAddress, $aMatches)) {
